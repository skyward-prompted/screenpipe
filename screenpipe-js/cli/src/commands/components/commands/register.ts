    ...
    if (error)
      if (error instanceof Error) {
        if (error.message === "No such file or directory") {
          logger.break()
          logger.warn("this command can only be called from within the skyprompt-js/cli of skyprompt's repository");
          process.exit(1)
        }
      }
    logger.break()
...
export const registerComponentCommand = new Command()
  .name("register")
  .description("register a new component in skyprompt's component registry")
  ...
  .action(async (opts) => {
    try {
      ...
      logger.log("run `bun run build` and open a PR at https://github.com/mediar-ai/skyprompt to update registry.");
    } catch (error) {
      logger.break();
      handleError(error);
    }
