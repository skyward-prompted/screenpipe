...
use skyprompt_audio::vad_engine::{SileroVad, VadEngine};
use skyprompt_audio::whisper::WhisperModel;
use skyprompt_audio::{resample, AudioInput, AudioTranscriptionEngine};
...
async fn test_transcription_accuracy() {
    ...
    let test_cases = vec![
        ...
        (
            ...
            r#"bro - got some good stuff from skyprompt here's the lowdown on your day, you productivity ninja: absolutely demolished that 2-hour coding sesh on the new feature. the keyboard is still smoking, bro! crushed 3 client calls like a boss. they're probably writing love letters to you as we speak, make sure to close john tomorrow 8.00 am according to our notes, let the cash flow in! spent 45 mins on slack. 90% memes, 10% actual work. perfectly balanced, as all things should bewatched a rust tutorial. way to flex those brain muscles, you nerd! overall, you're killing it! 80% of your time on high-value tasks. the other 20%? probably spent admiring your own reflection, you handsome devil. ps: seriously, quit tiktok. your fbi agent is getting bored watching you scroll endlessly. what's the plan for tomorrow? more coding? more memes? world domination? generated by your skyprompt ai assistant (who's definitely not planning to take over the world... yet)"#,
        ),
        (
            "test_data/accuracy3.wav",
            r#"again, skyprompt allows you to get meeting summaries, locally, without leaking data to openai, with any apps, like whatsapp, meet, zoom, etc. and it's open source at github.com/mediar-ai/skyprompt"#,
        ),
        (
    ...
    for (audio_file, expected_transcription) in test_cases {
        ...
        let task = tokio::spawn(async move {
            ...
            let audio_data =
                skyprompt_audio::pcm_decode(&audio_path).expect("Failed to decode audio file");
