...
# cli reference for skyprompt

<MotionDiv>

below is the detailed cli reference for skyprompt's core functionality.
...
- **ocr-engine** (`-o, --ocr-engine <ENGINE>`): OCR engine selection
  - options:
    ...
    - `custom`: configurable via `SKYPROMPT_CUSTOM_OCR_CONFIG`
...
# 2. configure skyprompt
export SKYPROMPT_CUSTOM_OCR_CONFIG='{
  "api_url": "http://localhost:8000/ocr",
  "api_key": "",
  "timeout_ms": 5000
}'

# 3. run skyprompt with custom ocr
skyprompt add \
  path/to/videos \
  --ocr-engine custom \
  --data-dir /tmp/sp
...
```bash
# list all pipes
skyprompt pipe list [--output <FORMAT>] [--port <PORT>]

# install a new pipe
skyprompt pipe install <URL> [--output <FORMAT>] [--port <PORT>]

# get pipe info
skyprompt pipe info <ID> [--output <FORMAT>] [--port <PORT>]

# enable/disable pipe
skyprompt pipe enable <ID> [--port <PORT>]
skyprompt pipe disable <ID> [--port <PORT>]

# delete pipe
skyprompt pipe delete <ID> [-y] [--port <PORT>]

# purge all pipes
skyprompt pipe purge [-y] [--port <PORT>]
```

#### add external data to skyprompt (OCR only)

allows you to add external screen recordings to skyprompt, for example it could be your iphone screen recordings, your physical journal photos, etc.

companies use this to index their product's screen recordings, for example.

```bash
# add video files
skyprompt add <PATH> [--data-dir <DIR>] [--output <FORMAT>] [--pattern <REGEX>] [--ocr-engine <ENGINE>] [--metadata-override <PATH>]
```

by default, skyprompt extracts metadata (fps, duration, creation time) directly from video files. however, you can override these with a metadata file:
...
find $HOME/.skyprompt/data -type f -name "*monitor*.mp4" | head -n 3 | while read -r file; do
    case $RANDOM in
        [0-9][0-9][0-9][0-9])
...
# 3. run skyprompt with metadata override
skyprompt add \
...
note: if you don't provide a metadata override file, skyprompt will automatically extract metadata from the video files. use overrides when you need to specify custom metadata or when the automatic extraction fails.
...
The `skyprompt` CLI supports generating shell completions for popular shells. Follow the steps below to enable autocompletion for your shell:  

#### Bash  
1. Add the following to your `.bashrc` profile:  
   ```bash
   eval "$(skyprompt completions bash)" 
   ```  
...
1. Add the following to your `.zshrc` profile:
   ```bash  
   eval "$(skyprompt completions zsh)"
   ```  
2. Reload your profile:  
   ```bash  
   source ~/.zshrc  
...
`
#### Fish  
1. Create `~/.config/fish/conf.d/skyprompt.fish` and add this line to it:
   ```fish  
   skyprompt completions fish | source  
   ```  

#### PowerShell  
1. Add the following to the end of your profile file:
   ```powershell  
   skyprompt completions powershell | Out-String | Invoke-Expression 
   ```
...
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-server/src/bin/skyprompt-server.rs
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-server/src/cli.rs
