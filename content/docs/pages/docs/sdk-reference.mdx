...
skyprompt provides two sdk packages:
- `@skyprompt/js` - for node.js environments (nextjs api routes, etc)
- `@skyprompt/browser` - for browser environments

both sdks provide type-safe interfaces to interact with skyprompt's core functionality.
...
<Tabs items={['npm', 'pnpm', 'bun', 'yarn']}>
  <Tab>
    ```bash copy
    npm install @skyprompt/js
    ```
  </Tab>
  <Tab>
    ```bash copy
    pnpm add @skyprompt/js
    ```
  </Tab>
  <Tab>
    ```bash copy
    bun add @skyprompt/js
    ```
  </Tab>
  <Tab>
    ```bash copy
    yarn add @skyprompt/js
    ```
  </Tab>
</Tabs>
...
<Tabs items={['npm', 'pnpm', 'bun', 'yarn']}>
  <Tab>
    ```bash copy
    npm install @skyprompt/browser
    ```
  </Tab>
  <Tab>
    ```bash copy
    pnpm add @skyprompt/browser
    ```
  </Tab>
  <Tab>
    ```bash copy
    bun add @skyprompt/browser
    ```
  </Tab>
  <Tab>
    ```bash copy
    yarn add @skyprompt/browser
    ```
  </Tab>
</Tabs>
...
const results = await pipe.querySkyprompt({
  ...
  browserUrl: "github.com/mediar-ai/skyprompt" // Filter by browser URL
})
```
...
check how the obsidian pipe implements this, [route](https://github.com/mediar-ai/skyprompt/blob/main/pipes/obsidian/src/app/api/log/route.ts) and [pipe.json](https://github.com/mediar-ai/skyprompt/blob/main/pipes/obsidian/pipe.json) for a complete example.
...
```bash
bunx @skyprompt/dev@latest components add
...
⚠️ make sure to enable real time transcription in the skyprompt app settings or CLI args.
...
pipes can access and modify skyprompt app settings through the SDK. this is useful for storing pipe-specific configuration and accessing global app settings.
...
- `get-skyprompt-app-settings` server action
...
```typescript
// src/lib/actions/get-skyprompt-app-settings.ts
import { pipe } from '@skyprompt/js'
import type { Settings as SkypromptAppSettings } from '@skyprompt/js'
...
export function usePipeSettings() {
  ...
  const loadSettings = async () => {
    try {
      ...
      const pipeSettings = {
        ...(skypromptSettings.customSettings?.yourPipeName && {
          ...skypromptSettings.customSettings.yourPipeName,
        }),
      }
...
- store pipe-specific settings under `customSettings.yourPipeName` in skyprompt app settings
...
see the [obsidian pipe](https://github.com/mediar-ai/skyprompt/tree/main/pipes/obsidian) for a complete example of settings management.



### integrating with llms

skyprompt's sdk can be easily integrated with various ai providers to analyze and generate insights from screen activity. here are common patterns for connecting context data with llms.

if the user is using skyprompt-cloud, you can use claude-3-7-sonnet, gemini-2.0-flash-lite or gpt-4o (Anthropic, Google, OpenAI or local models).
...
async function generateWorkLog(screenData, model, startTime, endTime) {
  ...
  const prompt = `You are analyzing screen recording data from Skyprompt.
...
see the [obsidian pipe](https://github.com/mediar-ai/skyprompt/tree/main/pipes/obsidian) for the complete implementation.
  ...
  for await (const chunk of pipe.streamTranscriptions()) {
    ...
    if (transcript.length > 200 && transcript.length % 30 === 0) {
      const response = await openai.chat.completions.create({
        model: "gpt-4o", // if user is using skyprompt-cloud, you can also use claude-3-7-sonnet or gemini-2.0-flash-lite
        messages: [
          {
            role: "system",
...
check out our [production pipe examples](https://github.com/mediar-ai/skyprompt/tree/main/pipes) on github to see more ai integration patterns.
...
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-js/node-sdk/src/index.ts
