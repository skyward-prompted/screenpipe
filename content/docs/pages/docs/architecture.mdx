...
skyprompt's architecture handles continuous screen and audio capture, local data storage, and real-time processing. here's a breakdown of the key components:
...
at its core, skyprompt acts as a bridge between your digital activities and AI systems, creating a memory layer that provides context for intelligent applications. here's how to think about it:
...
![skyprompt diagram](https://raw.githubusercontent.com/mediar-ai/skyprompt/main/content/diagram2.png)
...
this modular architecture makes skyprompt adaptable to various use cases, from personal productivity tracking to advanced business intelligence.

<MotionDiv delay={0.2}>

### data flow & lifecycle

here's the typical data flow through the skyprompt system:
...
![skyprompt data abstractions](https://raw.githubusercontent.com/user-attachments/assets/93136194-0945-4eec-a9f1-f58eb9e440a4)

skyprompt organizes data in concentric layers of abstraction, from raw data to high-level intelligence:
...
skyprompt maintains several types of state:

1. **session state**
   - managed by the core skyprompt server
...
3. **pipe state**
   ...
   - stored in the pipe's local storage or in skyprompt's settings
...
skyprompt uses a SQLite database with the following main tables:
...
```bash
sqlite3 ~/.skyprompt/db.sqlite .schema
```

### integration patterns

developers typically interact with skyprompt in one of these patterns:

1. **retrieval pattern**: query for relevant context based on the current task
   ```typescript
   const context = await pipe.querySkyprompt({
     q: "meeting notes",
     contentType: "all",
     limit: 10
   })
...
3. **augmentation pattern**: enhance user experience with context
   ...
   const meetingContext = await pipe.querySkyprompt({
     q: "meeting",
     contentType: "audio"
   })
...
understanding these patterns will help you design effective applications that leverage skyprompt's capabilities.
...
- [ ] Integrations
    ...
    - [ ] Keyboard
    - [x] Browser
    - [x] Pipe Store (a list of "pipes" you can build, share & easily install to get more value out of your screen & mic data without effort). It runs in Bun Typescript engine within skyprompt on your computer
- [x] screenshots + OCR with different engines to optimise privacy, quality, or energy consumption
  - [x] tesseract
  - [x] Windows native OCR
  - [x] Apple native OCR
  - [x] unstructured.io
  - [ ] skyprompt screen/audio specialised LLM
- [x] audio + STT (works with multi input devices, like your iPhone + mac mic, many STT engines)
  ...
  - [x] iPhone microphone
- [x] [remote capture](https://github.com/mediar-ai/skyprompt/discussions/68) (run skyprompt on your cloud and it capture your local machine, only tested on Linux) for example when you have low compute laptop
...
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-server/src/core.rs
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-audio/src/core.rs
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-vision/src/core.rs
