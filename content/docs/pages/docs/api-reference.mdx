...
# api reference for skyprompt

we highly recommend just copy pasting these into your cursor chat for context:

- [server API](https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-server/src/server.rs)
- [app API](https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-app-tauri/src-tauri/src/server.rs)

and ask questions as needed.

<MotionDiv>

below is the detailed api reference for skyprompt's core functionality.
...
- **endpoint**: `/search`
- **method**: `get`
- **description**: searches captured data (ocr, audio transcriptions, etc.) stored in skyprompt's local database.
...
- **url filtering**: use the browser_url parameter to filter content from specific websites
  ...
  curl "http://localhost:3030/search?content_type=ocr&browser_url=github.com/mediar-ai/skyprompt"
  ```
...
- https://github.com/mediar-ai/skyprompt/blob/main/skyprompt-server/src/db.rs
